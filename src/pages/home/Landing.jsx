...
export default function Landing() {
  ...
  const specialties = [
    ...
    { icon: <Users />, label: "نساء وتوليد" },
    {
      icon: <HandMetal />,
      label: "جلدية",
      href: "https://www.figma.com/design/e8D4AKIrGCyGL3yDPdLru2/Untitled?node-id=40-10008&t=XrqsbZO6Bmz70g7q-4",
    },
  ];
  ...
  const faqs = [
    ...
    {
      question: "هل أحتاج لتثبيت تطبيق؟",
      answer: "لا، يمكنك استخدام المنصة مباشرة من خلال المتصفح.",
    },
  ];

  return (
    /* Responsive Fix: overflow-x-hidden prevents horizontal scroll on all screen sizes */
    <div className="min-h-screen bg-[#F7FBFF] overflow-x-hidden">
      ...
      <section id="home" className="relative overflow-hidden">
        ...
        {/* Responsive Fix: reduced vertical padding on mobile (py-10), full on lg+ (py-20) */}
        <div className="container mx-auto px-4 py-10 lg:py-20 relative z-10">
          {/* Responsive Fix: single column on mobile, two columns on lg+ */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">

            {/* Text block */}
            <div className="text-right space-y-6 lg:space-y-8">
              {/* Responsive Fix: scale heading from 4xl on mobile up to 7xl on xl */}
              <h1 className="font-changa text-4xl sm:text-5xl md:text-6xl xl:text-7xl font-semibold text-[#468EEC] leading-tight">
                مرحبا بك في منصة رفيق
              </h1>

              <div className="space-y-4 lg:space-y-6">
                {/* Responsive Fix: scale sub-heading from 2xl on mobile up to 5xl on xl */}
                <h2 className="font-changa text-2xl sm:text-3xl md:text-4xl xl:text-5xl font-semibold text-[#121212]">
                  ابدأ رحلتك الصحية بسهولة
                </h2>
                {/* Responsive Fix: slightly smaller body text on mobile */}
                <p className="text-base sm:text-lg lg:text-xl font-cairo text-gray-600 leading-relaxed">
                  منصّة رفيق تسهّل عليك حجز مواعيد الأطباء في أي وقت وبخطوات بسيطة.
                  وبوت ذكي يساعدك تفهم أعراضك ويختار لك التخصص المناسب بسرعة.
                </p>
              </div>

              {/* Responsive Fix: wrap buttons on very small screens, reduced px on mobile */}
              <div className="flex flex-wrap gap-4 lg:gap-10 justify-end">
                <Button
                  variant="primary"
                  size="lg"
            ...
            {/* Hero image */}
            {/* Responsive Fix: hidden on mobile to prevent layout break; shown from lg upward */}
            <div className="hidden lg:flex justify-center relative h-[500px]">
              <div className="absolute left-1/2 -translate-x-1/2 top-0 w-[380px] z-20">
                <img
                  src={drTaha}
                  alt="doctor"
                  /* Responsive Fix: ensure image never overflows its container */
      ...
      {/* ─── Rafiq Assistant Section ─── */}
      <section
        id="assistant"
        /* Responsive Fix: replaced fixed px-20 with responsive padding scale */
        className="w-full py-12 lg:py-20 px-4 sm:px-8 md:px-12 lg:px-20"
        style={{
          background: "linear-gradient(180deg, #003B88 45%, #1DA1F2 100%)",
        }}
      >
        {/* Responsive Fix: column on mobile, row on lg+ */}
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center justify-between gap-8 lg:gap-10">

          {/* Rafiq image */}
          {/* Responsive Fix: fluid width capped at reasonable sizes per breakpoint */}
          <div className="w-[200px] sm:w-[260px] md:w-[300px] lg:w-[400px] flex-shrink-0">
            <img
              src={rafiqHeart}
              alt="rafiq"
              className="w-full object-contain"
          ...
          {/* Text block */}
          <div className="text-white text-right max-w-xl w-full">
            {/* Responsive Fix: scale heading from 2xl on mobile to 5xl on md+ */}
            <h2 className="text-2xl sm:text-3xl md:text-5xl font-changa font-bold leading-[1.4] mb-4 lg:mb-6">
              اعرف حالتك... خطوة<br />
              بخطوة مع رفيقك الصحي
            </h2>

            <p className="text-sm md:text-base font-cairo leading-7 opacity-90 mb-6 lg:mb-8">
              منصة رفيق تسهّل عليك حجز مواعيد الأطباء في أي وقت وبخطوات بسيطة، وبوت ذكي يساعدك
              رفيقك الصحي هيساعدك تفهم الأعراض اللي بتحس بيها، ويعرّفك التخصص المناسب لحالتك،
              ويقدّم لك إرشادات أولية تساعدك تاخد القرار الصح.
            </p>

            {/* Responsive Fix: wrap buttons on small screens */}
            <div className="flex gap-4 justify-end flex-wrap">
              <button
                className="bg-[#5C8DF6] hover:opacity-90 text-white px-5 py-3 rounded-lg font-cairo text-sm sm:text-base"
                onClick={() => navigate("/login")}
              >
                ابدأ التجربة الآن
              </button>
              <button
                className="bg-white text-[#468EEC] px-5 py-3 rounded-lg font-cairo border text-sm sm:text-base"
                onClick={() => navigate("/login")}
              >
                كيف يعمل رفيقك الصحي؟
              </button>
      ...
      {/* ─── Booking Section ─── */}
      <section id="booking" className="py-12 lg:py-20">
        <div className="container mx-auto px-4">

          <div className="text-center mb-8 lg:mb-12">
            {/* Responsive Fix: scale heading from 3xl on mobile to 6xl on lg+ */}
            <h2 className="font-changa text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-semibold text-[#468EEC] mb-4">
              احجز موعد مع طبيبك
            </h2>
            <p className="text-base sm:text-lg lg:text-xl font-cairo text-gray-600">
              ابحث عن التخصص أو الطبيب وحدد الوقت المناسب لك
            </p>
          </div>

          <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-lg p-4 sm:p-6 lg:p-8">
            {/* Responsive Fix: stacked on mobile, 3-column on md+ */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6 mb-6">

              <div className="flex items-center gap-4 p-4 border border-gray-200 rounded-xl">
                <Stethoscope className="w-6 h-6 text-[#468EEC] flex-shrink-0" />
                <span className="font-cairo text-gray-600">التخصص</span>
              </div>

              <div className="flex items-center gap-4 p-4 border border-gray-200 rounded-xl">
                <MapPin className="w-6 h-6 text-[#468EEC] flex-shrink-0" />
                <span className="font-cairo text-gray-600">المكان</span>
              </div>

              <div className="flex items-center gap-4 p-4 border border-gray-200 rounded-xl">
                <Clock className="w-6 h-6 text-[#468EEC] flex-shrink-0" />
                <span className="font-cairo text-gray-600">الوقت</span>
              </div>
      ...
      <section
        dir="rtl"
        /* Responsive Fix: replaced hard-coded px-[100px] with responsive horizontal padding */
        className="w-full px-4 sm:px-8 md:px-12 lg:px-16 xl:px-[100px] py-8 bg-[#F7FBFF] rounded-[16px] mb-12"
      >
        {/* Title */}
        <div className="text-center mb-8 lg:mb-12">
          {/* Responsive Fix: scale heading from 3xl on mobile to 6xl on lg+ */}
        ...
        {/* Responsive Fix: single column on mobile, 2 on md, 3 on lg */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {doctors.map((doctor) => (
            <DoctorCard
              key={doctor.id}
      ...
      {/* ─── Specialties Section ─── */}
      <section id="specialties" className="py-12 lg:py-20 bg-white">
        <div className="container mx-auto px-4">
          ...
          <div className="flex justify-center mb-10 lg:mb-16">
            {/* Responsive Fix: full-width on mobile, capped at 726px on larger screens */}
            <div className="w-full max-w-[726px] relative">
              <input
                type="text"
                placeholder="البحث"
          ...
          {/* Specialties Grid */}
          {/* Responsive Fix: 2 cols on mobile, 3 on md, 4 on lg — removed fixed w-[292px] h-[187px] */}
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-6 justify-items-center">
            {specialties.map((specialty, index) => (
              /* Responsive Fix: fluid card width with aspect ratio instead of fixed pixel dimensions */
              <div
                key={index}
                className="w-full max-w-[292px] aspect-[292/187] bg-white border border-[#468EEC] rounded-[24px] flex flex-col items-center justify-center gap-3 lg:gap-4 hover:shadow-md transition"
              >
                {/* Responsive Fix: scale icon size down on mobile */}
                <div className="text-[#468EEC] text-3xl sm:text-4xl lg:text-5xl">
                  {specialty.icon}
                </div>
                {/* Responsive Fix: scale label from xl on mobile to 3xl on lg */}
                <p className="font-changa text-xl sm:text-2xl lg:text-[32px] font-semibold text-[#468EEC] text-center">
                  {specialty.label}
                </p>
              </div>
            ))}
      ...
      <section className="py-12 lg:py-20 bg-white">
        <div className="max-w-[1240px] mx-auto px-4">
          ...
          <div
            /* Responsive Fix: replaced fixed px-[73px] with responsive padding; column on mobile, row on lg */
            className="w-full rounded-[24px] px-6 sm:px-10 md:px-12 lg:px-[73px] py-8 lg:py-[37px] flex flex-col lg:flex-row items-center justify-between gap-8 lg:gap-10 shadow-lg"
            style={{
              background: "linear-gradient(180deg, #003B88 45%, #1DA1F2 100%)",
              boxShadow: "0px 8px 20px rgba(0,0,0,0.25)",
            }}
          >
            {/* Rafiq image */}
            {/* Responsive Fix: fluid width per breakpoint */}
            <div className="w-[180px] sm:w-[220px] md:w-[280px] lg:w-[320px] flex-shrink-0">
              <img
                src={rafiqHeart}
                alt="rafiq"
                className="w-full object-contain drop-shadow-lg"
            ...
            {/* Text block */}
            <div className="text-white text-right max-w-xl w-full">
              {/* Responsive Fix: scale from 2xl on mobile to 5xl on md+ */}
              <h2 className="font-changa text-2xl sm:text-3xl md:text-5xl font-semibold leading-[1.4] mb-4 lg:mb-6">
                مش عارف تختار التخصص؟<br />
                رفيق يساعدك!
              </h2>

              <p className="font-cairo text-sm md:text-base opacity-90 mb-6 lg:mb-8">
                احكي لرفيق أعراضك، وهو يرشح لك التخصص المناسب
              </p>

              <button
                className="bg-white text-[#468EEC] px-6 py-3 rounded-lg font-cairo hover:opacity-90 transition text-sm sm:text-base"
                onClick={() => navigate("/login")}
              >
                ابدأ التشخيص المبدئي
              </button>
      ...
      <section className="py-12 lg:py-20">
        <div className="container mx-auto px-4">
          ...
          {/* Responsive Fix: full-width on mobile, capped at 4xl on larger screens */}
          <div className="max-w-4xl mx-auto space-y-4">
            {faqs.map((faq, index) => (
              <FAQItem key={index} {...faq} />
            ))}
